...
import { db, auth as authentication } from "../../firebase/firebase";
import "./Login.css";
  ...
  const preSignup = async (e) => {
    ...
    if (error === false) {
      try {
        setSignupState(true)
        e.preventDefault()
        const res = await createUserWithEmailAndPassword(authentication, signupState.email, signupState.password);
        const user = res.user;
